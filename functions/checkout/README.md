...
Then `wrangler deploy`. In CI, deployment is a separate job gated on the `main` branch and on the four
secrets (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`)
being present.
