...
Only Regular (400) and Bold (700) faces of Space Grotesk are shipped. Headings that asked for an in-between
weight — `Print details` on the product page, the card title, the checkout order summary heading, and the sheet
title (the cart drawer's "Your cart", the mobile menu's "Formline") — were changed to `font-bold` or `font-normal`
so the requested weight matches a face that is actually shipped, instead of silently falling back to 400 or 700
under the browser's font-matching rules.
